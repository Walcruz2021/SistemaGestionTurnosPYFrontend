 ...
 * @param {*} listSupplies (array of purchased supplies)
 * @param stateListSupplies
 * @param setStateListSupplies 
 * @param stateActiveTable display the table (true) does not display the table (false)
 * @param setStateActiveTable
 * @param setSupplySelectedFromTable  Remove the filter so that the variants do not appear in the variants table.
 * @returns setStateListSupplies filtered by parameters. Is used in FormAddSale
 */

const SearchSupplyAddSale = ({ listSupplies, stateListSupplies, setStateListSupplies, stateActiveTable, setStateActiveTable, setSupplySelectedFromTable }) => {
    ...
    const suppliesFiltered = useMemo(() => {
        let result = Array.isArray(listSupplies)
            ? [...listSupplies]
            : [];
        ...
        if (categoryToSearch) {
            ...
            result = result.filter(
                (s) => s?.global?.idCategory === categoryToSearch.value
            );
        }
    ...
    useEffect(() => {
        if (suppliesFiltered) {
            ...
            setStateListSupplies(suppliesFiltered)
        }
    }, [suppliesFiltered])
    ...
    const handleChangeSelectCategory = (selected) => {
        ...
        setStateActiveTable(false)
        setSupplySelectedFromTable(null)
    };
...
export default SearchSupplyAddSale