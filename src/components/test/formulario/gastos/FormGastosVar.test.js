...
import FormGastosVar from "../../../Formulario/Gastos/FormGastosVar";
const mockStore = configureStore([]);

describe("FormGatosVar", () => {
  let store;
  beforeEach(() => {
    store = mockStore({
  ...
  test("El botón Guardar Gasto se habilita solo si los campos obligatorios están completos", async () => {
    ...
    await user.click(await screen.findByText("Mano de Obra Tecnico"));
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "Ingresar Gasto Variable" })
      ).toBeEnabled()
    );
  });
});
