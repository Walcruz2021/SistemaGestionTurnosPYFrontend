...
import FormGastosInd from "../../../Formulario/Gastos/FormGastosInd";
...
describe("FormGatosInd", () => {
  ...
  test("El botón Guardar Gasto se habilita solo si los campos obligatorios están completos", async () => {
    ...
    await user.click(await screen.findByText("Gastos de Gestion"));
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "Ingresar Gasto Indirecto" })
      ).toBeEnabled()
    );
